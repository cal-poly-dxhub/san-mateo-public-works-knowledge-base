    ...
    try:
        ...
        try:
            response = bedrock_client.start_ingestion_job(
                ...
                dataSourceId=get_data_source_id(kb_id)
            )
            logger.info(f"Started KB ingestion job: {response.get('ingestionJob', {}).get('ingestionJobId')}")
        except Exception as e:
            logger.warning(f"Could not trigger KB sync: {e}. Sync will happen on schedule.")
