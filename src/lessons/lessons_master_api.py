        ...
        for project_type, data in project_types.items():
            ...
            for project_name in data['projects']:
                try:
                    ...
                    lesson_files = [obj for obj in s3_response.get('Contents', []) 
                                  if obj['Key'].endswith('.json')]
                    lesson_count += len(lesson_files)
                    
                    print(f"Found {len(lesson_files)} lessons for {project_name}")
            ...
            try:
                ...
                for obj in s3_response.get('Contents', []):
                    if obj['Key'].endswith('.json'):
                        ...
                        # Add metadata
                        lesson_data['id'] = obj['Key'].split('/')[-1].replace('.json', '')
...
def update_lesson(lesson_id, lesson_data):
    ...
    try:
        ...
        lesson_content = {k: v for k, v in lesson_data.items() 
