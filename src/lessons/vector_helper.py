...
def trigger_project_lessons_ingestion(bucket_name: str, project_name: str):
    """Trigger vector sync for project lessons file by invoking async processor"""
    import json
...
def delete_vectors_by_project(
    vector_bucket_name: str, index_name: str, project_name: str
):
    """
    ...
    try:
        ...
        if vector_ids:
            ...
            print(
                f"Deleted {len(vector_ids)} vectors for project {project_name}"
            )
        else:
...
def delete_vectors_by_file(
    vector_bucket_name: str, index_name: str, project_name: str, file_name: str
):
    """
    ...
    try:
        ...
        if vector_ids:
            ...
            print(
                f"Deleted {len(vector_ids)} vectors for {project_name}/{file_name}"
            )
...
def query_vectors_by_project(
    ...
    try:
        response = s3vectors_client.query_vectors(
            ...
            metadataFilters={"project_name": {"equals": project_name}},
...
def query_vectors_by_type(
    ...
    try:
        response = s3vectors_client.query_vectors(
            ...
            indexName=index_name,
            queryVector={"float32": query_embedding},
            metadataFilters={"project_type": {"equals": project_type}},
            maxResults=max_results,
        )
