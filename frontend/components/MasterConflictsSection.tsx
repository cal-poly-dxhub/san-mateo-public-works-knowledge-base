...
interface MasterConflictsSectionProps {
  projectType: string;
  onResolved: () => void;
}

export default function MasterConflictsSection({ projectType, onResolved }: MasterConflictsSectionProps) {
  ...
  const resolveConflict = async (conflictId: string, decision: string) => {
    try {
      await apiRequest(`/lessons/conflicts/resolve/${encodeURIComponent(conflictId)}`, {
        method: "POST",
        body: JSON.stringify({ decision, project_type: projectType }),
      });
